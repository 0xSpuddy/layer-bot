...
# Debug the URL specifically
click.echo("\nDebug - RPC URL analysis:")
click.echo(f"Raw URL: {repr(layer_rpc_url)}")  # Use repr() to show hidden characters
click.echo(f"URL length: {len(layer_rpc_url)}")
click.echo(f"URL characters: {[ord(c) for c in layer_rpc_url]}")  # Show ASCII values
...
@click.group()
def tx():
    """Transaction commands for Layer chain."""
    pass

@tx.command('claim-deposits')
def claim_deposits():
